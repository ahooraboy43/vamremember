...
function getFilteredItems() {
  ...
  return allExpenses.filter(i => {
    ...
    // پرداخت‌شده‌های این ماه: هم اقساط و هم هزینه‌ها
    if (activeFilter === "paid" && !isPaidValue(i[currentMonthKey])) {
      return false;
    }

    // پرداخت‌نشده‌های این ماه: فقط اقساط
    if (
      activeFilter === "unpaid" &&
...
function createAllItemCard(item){
  ...
  const secondary=expense?`جمع کل: ${formatMoney(getExpenseTotal(item))}`:`${getRemainingInstallments(item).toLocaleString("fa-IR")} ظظظ‌مانده`;
...
function openEditModal(item){resetExpenseForm();editExpenseId.value=item.id;expenseTitle.value=item.title||"";expenseNote.value=item.note||"";const type=isExpense(item)?"expense":"installment";setExpenseType(type);if(type==="installment"){expenseAmount.value=item.amount||"";expenseDueDay.value=item.due_day||"";expenseInstallments.value=item.installment_count||""}else expenseAmount.value=getExpenseAmount(item);expenseStartMonth.value=findStartMonth(item);expenseModalTitle.textContent=type==="installment"?"ویرایش قسط":"ویرایش هزینه";buildMonthEditor(item);monthsEditor.classList.remove("hidden");saveExpenseButton.textContent="ذخیره تغییرات";openModal()}
...
function buildEditBody(type){const body={title:expenseTitle.value.trim(),note:expenseNote.value.trim()||null};if(type==="installment"){body.amount=Number(expenseAmount.value);body.due_day=Number(expenseDueDay.value);body.installment_count=Number(expenseInstallments.value)}else{body.amount=null;body.due_day=null;body.installment_count=null}monthFields.querySelectorAll("[data-month]").forEach(input=>body[input.dataset.month]=input.value.trim()===""?null:input.value.trim());return body}
...
function getReportItems(kind){
  ...
  if(kind==="expenses")return expenses.filter(i=>isPaidValue(i[currentMonthKey]));
...
function openPage(id,title){pages.forEach(p=>p.classList.toggle("active",p.id===id));navButtons.forEach(b=>b.classList.toggle("active",b.dataset.page===id));pageTitle.textContent=title;if(id==="allPage")renderAllCards();if(id==="reportPage")renderReports();window.scrollTo({top:0,behavior:"smooth"})}
...
updatePersianDate();expenseStartMonth.value=currentMonthKey;setExpenseType("installment");loadData();
